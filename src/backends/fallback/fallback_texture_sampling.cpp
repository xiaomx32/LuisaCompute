...
void luisa_bc7_read(const FallbackTextureView* tex, uint x, uint y, float4& out) noexcept
{
    ...
    const bc::BC7Block * bc_block = reinterpret_cast<const bc::BC7Block *>(tex->data()) + (block_pos.x + block_pos.y * block_per_row);
...
[[nodiscard]] inline auto texture_sample_point(FallbackTextureView* view, Sampler::Address address, const float2& uv) noexcept {
...
[[nodiscard]] inline auto texture_sample_linear(FallbackTextureView* view, Sampler::Address address, const float2& uv) noexcept
{
    auto size = make_float2(view->size2d());
...
float4 bindless_texture_2d_sample_level(const FallbackTexture *tex, uint sampler, float u, float v, float lod) noexcept
{
    auto s = Sampler::decode(sampler);
...
float4 bindless_texture_2d_sample_grad(const FallbackTexture *tex, uint sampler, float u, float v, int64_t dpdx, int64_t dpdy) noexcept {
    return bindless_texture_2d_sample(tex, sampler, u, v);
}
...
float4 fallback::bindless_texture_3d_sample(const FallbackTexture *tex, uint sampler, float u, float v, float w) noexcept
{
    auto s = Sampler::decode(sampler);
    auto view = tex->view(0);
    return s.filter() == Sampler::Filter::POINT ?
...
float4 bindless_texture_3d_sample_level(const FallbackTexture *tex, uint sampler, float u, float v, float w, float lod) noexcept {
...
//swfly: im too lazy to do this. complete it someday
float4 bindless_texture_3d_sample_grad(const FallbackTexture *tex, uint sampler, float u, float v, float w, int64_t dudxy, int64_t dvdxy, int64_t dwdxy) noexcept
{
    return bindless_texture_3d_sample(tex, sampler, u, v, w);
}
