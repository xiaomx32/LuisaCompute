...
float4 fallback::bindless_texture_2d_sample(const FallbackTexture *tex, uint sampler, float u, float v) noexcept {
    ...
    return s.filter() == Sampler::Filter::POINT ? texture_sample_point(&view, s.address(), make_float2(u, v)) : texture_sample_linear(&view, s.address(), make_float2(u, v));
}
...
float4 fallback::bindless_texture_3d_sample(const FallbackTexture *tex, uint sampler, float u, float v,
    ...
    auto s = Sampler::decode(sampler);
    auto view = tex->view(0);
    return s.filter() == Sampler::Filter::POINT ? texture_sample_point(&view, s.address(), make_float3(u, v, w)) : texture_sample_linear(&view, s.address(), make_float3(u, v, w));
}
