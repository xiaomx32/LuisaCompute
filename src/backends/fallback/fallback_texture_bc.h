...
class HDRColorA;
    ...
    class HDRColorA
    {
        ...
        static void InterpolateRGB(_In_ const LDRColorA& c0, _In_ const LDRColorA& c1, _In_ size_t wc, _In_ _In_range_(2, 4) size_t wcprec, _Out_ LDRColorA& out) noexcept
        {
            ...
            switch (wcprec)
            {
                case 2: aWeights = g_aWeights2; LUISA_ASSUME(wc < 4); _Analysis_assume_(wc < 4); break;
                case 3: aWeights = g_aWeights3; LUISA_ASSUME(wc < 8); _Analysis_assume_(wc < 8); break;
                case 4: aWeights = g_aWeights4; LUISA_ASSUME(wc < 16); _Analysis_assume_(wc < 16); break;
                default: LUISA_ASSUME(false); out.r = out.g = out.b = 0; return;
            }
        ...
        static void InterpolateA(_In_ const LDRColorA& c0, _In_ const LDRColorA& c1, _In_ size_t wa, _In_range_(2, 4) _In_ size_t waprec, _Out_ LDRColorA& out) noexcept
        {
            ...
            switch (waprec)
            {
                case 2: aWeights = g_aWeights2; LUISA_ASSUME(wa < 4); _Analysis_assume_(wa < 4); break;
                case 3: aWeights = g_aWeights3; LUISA_ASSUME(wa < 8); _Analysis_assume_(wa < 8); break;
                case 4: aWeights = g_aWeights4; LUISA_ASSUME(wa < 16); _Analysis_assume_(wa < 16); break;
                default:
                    LUISA_ASSUME(false);
                    out.a = 0;
                    return;
