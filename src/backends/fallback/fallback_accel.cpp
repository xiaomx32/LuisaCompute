...
void intersect_closest_wrapper(void *accel,
                               float ox, float oy, float oz,
                               float dx, float dy, float dz,
                               float tmin, float tmax,
                               unsigned mask, void *hit) {
    luisa::compute::fallback::accel_trace_closest(
        static_cast<luisa::compute::fallback::FallbackAccel *>(accel),
        ox, oy, oz, dx, dy, dz, tmin, tmax, mask, reinterpret_cast<luisa::compute::SurfaceHit *>(hit));
}

bool intersect_any_wrapper(void *accel,
                           float ox, float oy, float oz,
                           float dx, float dy, float dz,
                           float tmin, float tmax,
                           unsigned mask) {
    return luisa::compute::fallback::accel_trace_any(
        static_cast<luisa::compute::fallback::FallbackAccel *>(accel),
        ox, oy, oz, dx, dy, dz, tmin, tmax, mask);
}
